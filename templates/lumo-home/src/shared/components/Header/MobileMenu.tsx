...
export function MobileMenuSheet({ user }: { user: User | null | undefined }) {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent className="flex flex-col max-h-dvh! w-full sm:max-w-lg">
        <SheetHeader>
          <SheetTitle className="mt-4 xl:mt-8 fl-text-32/48">Меню</SheetTitle>
        </SheetHeader>
        <div className="grow">
          <ScrollArea className="grow min-h-0! max-h-full!">
            {user ? (
