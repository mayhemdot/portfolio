...
export function InputField<S>(params: Props<S>) {
	const {
		label,
		name,
		hidden = false,
						...
						{link?.href && (
							<Link
								...
								className={btnVariants({
									variant: "link",
									className: cn("p-0! text-blue-600! ml-auto h-fit", {
										"hidden!":
											type !== "password" || name.toLowerCase() !== "password",
									}),
								})}
							>
								{link.label}
					...
					<div className='relative'>
						...
						{type === "password" && (
							<button
								...
								className='absolute right-0 top-0 mr-4 mt-4 focus:outline-none'
								type='button'
