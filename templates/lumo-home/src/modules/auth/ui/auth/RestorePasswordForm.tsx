...
export function RestorePasswordFormClient() {
	...
	const [code] = useQueryState("restore_code", parseAsString.withDefault(""));
	...
	const handleRestorePassword = async (e: React.FormEvent<HTMLFormElement>) => {
		e.preventDefault();
		setErrors({});
	...
	return (
		<form onSubmit={handleRestorePassword} className='space-y-4'>
			...
			<InputField label='Code' type='text' name='code' value={code} disabled />

			<Button type='submit' disabled={isLoading} className='w-full'>
				Restore Password
			</Button>

			{errors?.root && <p className='py-4 text-red-600'>{errors.root}</p>}
		</form>
	);
}
