...
export const PromoRoundBlock: React.FC<PromoRoundBlockProps> = ({
  ...
  return (
    <div
      ref={sectionRef}
      id={`promo-block-${id}`}
      className="relative flex h-screen overflow-clip  w-full items-center justify-center"
    >
      {introContent && (
        <div
          ref={wrapperRef}
        ...
        >
          ...
          <span className="absolute left-1/2   top-1/2 z-20  w-[150%] -translate-x-1/2 -translate-y-1/2">
            ...
            <p
              ref={textRef}
              className="promo-round-text fl-text-28/80 text-dark translate-y-5 whitespace-pre-line text-center font-medium leading-tight opacity-0 blur-2xl will-change-transform"
            >
              {introContent}
            </p>
          </span>
