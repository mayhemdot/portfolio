...
export const seed = async (payload: Payload): Promise<void> => {
  ...
  const product1Doc = await payload.create({
    collection: "products",
    data: JSON.parse(JSON.stringify({ ...product1, categories: [apparelCategory.id] }).replace(/"\{\{PRODUCT_IMAGE\}\}"/g, String(image1ID))),
  });

  const product2Doc = await payload.create({
    collection: "products",
    data: JSON.parse(JSON.stringify({ ...product2, categories: [ebooksCategory.id] }).replace(/"\{\{PRODUCT_IMAGE\}\}"/g, String(image2ID))),
  });

  const product3Doc = await payload.create({
    collection: "products",
    data: JSON.parse(JSON.stringify({ ...product3, categories: [coursesCategory.id] }).replace(/"\{\{PRODUCT_IMAGE\}\}"/g, String(image3ID))),
  });
  ...
  await Promise.all([
    await payload.update({
      ...
      id: product1Doc.id,
      data: {
        relatedProducts: [Number(product2Doc.id), Number(product3Doc.id)],
      },
    }),
    await payload.update({
      collection: "products",
      id: product2Doc.id,
      data: {
        relatedProducts: [Number(product1Doc.id), Number(product3Doc.id)],
      },
    }),
    await payload.update({
      collection: "products",
      id: product3Doc.id,
      data: {
        relatedProducts: [Number(product1Doc.id), Number(product2Doc.id)],
      },
    }),
  ]);
  ...
  await payload.updateGlobal({
    slug: "settings",
    data: {
      productsPage: productsPageDoc.id ? Number(productsPageDoc.id) : null,
    },
  });
      ...
      navItems: [
        {
          link: {
            ...
            reference: {
              relationTo: "pages",
              value: Number(productsPageDoc.id),
            },
            label: "Shop",
          },
        },
